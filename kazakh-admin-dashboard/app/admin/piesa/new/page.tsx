...
import { useCreatePlay, useUploadFile } from "@/lib/api"
import { createPlaySchema } from "@/lib/validation"
...
const categories = [
  ...
  { value: 'historical', label: 'Тарихи' },
  { value: 'modern', label: 'Заманауи' },
  { value: 'classical', label: 'Классикалық' },
  { value: 'experimental', label: 'Эксперименталды' },
]
...
function NewPlayForm() {
  ...
  const createPlayMutation = useCreatePlay()
  const uploadFileMutation = useUploadFile()
  const { validate, getFieldError, clearErrors } = useValidation(createPlaySchema)
  ...
  const handlePublish = async () => {
    ...
    const validation = validate(playData);
    ...
    try {
      ...
      const finalPlayData = { ...playData, imageUrl: finalImageUrl || undefined };
      await createPlayMutation.mutateAsync(finalPlayData);
      toast({ title: "Сәтті!", description: "Пьеса сәтті жарияланды" });
      router.push("/admin/piesa");
    } catch (error: any) {
      toast({ title: "Қате!", description: error.message || "Пьеса жариялау кезінде қате орын алды", variant: "destructive" });
    }
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const finalPlayData = { ...playData, imageUrl: finalImageUrl || undefined };
      await createPlayMutation.mutateAsync(finalPlayData);
      toast({ title: "Сәтті!", description: "Пьеса жоба ретінде сақталды" });
      router.push("/admin/piesa");
    } catch (error: any) {
      toast({ title: "Қате!", description: error.message || "Пьесаны сақтау кезінде қате орын алды", variant: "destructive" });
    }
  };

  const isLoading = createPlayMutation.isPending || uploadFileMutation.isPending
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Жаңа пьеса қосу</h1>
          <p className="text-gray-600 mt-1">Пьеса ақпаратын толтырыңыз</p>
        </div>
      </div>
            ...
            <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Пьеса атауын енгізіңіз"
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="Пьеса мазмұнын жазыңыз..."
...
export default dynamic(() => Promise.resolve(NewPlayForm), {
  ssr: false,
}) 