...
import { useReview, useUpdateReview, useUploadFile } from "@/lib/api"
...
export default function EditReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: review, isLoading: isLoadingReview } = useReview(id);
  const updateReviewMutation = useUpdateReview();
  const uploadFileMutation = useUploadFile();

  useEffect(() => {
    if (id === "admin") {
      router.replace("/admin/synshy");
    }
  }, [id, router]);
  ...
  const handleSave = async () => {
    try {
      ...
      await updateReviewMutation.mutateAsync({ id: id, data: reviewData })
      
      toast({
        title: "Сәтті!",
        description: "Рецензия сәтті жаңартылды",
      })
      ...
      toast({
        title: "Қате!",
        description: error.message || "Рецензия жаңарту кезінде қате орын алды",
        variant: "destructive",
      })
    }
  }
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Рецензияны өңдеу</h1>
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="text-center py-12">
          <p className="text-red-600 mb-4">Рецензия табылмады</p>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Рецензияны өңдеу</h1>
          <p className="text-gray-600 mt-1">Рецензия #{id}</p>
        </div>
      </div>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="pt-6">
              <div className="space-y-3">
                <Button 
                  onClick={handleSave} 
                  disabled={updateReviewMutation.isPending || uploadFileMutation.isPending || !title || !content}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white text-base py-3"
                >
                  {updateReviewMutation.isPending || uploadFileMutation.isPending ? "Сақталуда..." : (
                    <>
                ...
                <Button 
                  ...
                  disabled={updateReviewMutation.isPending || uploadFileMutation.isPending}
                >
